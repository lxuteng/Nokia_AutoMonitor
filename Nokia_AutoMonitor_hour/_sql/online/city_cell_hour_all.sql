...
,round(sum(平均激活用户数),2) 平均激活用户数
,sum(最大激活用户数)  最大激活用户数       
,sum(RRC最大连接数)  RRC最大连接数 
,sum(M8008C4 + M8013C17 + M8013C18 + M8013C19 + M8013C20 + M8013C21)  RRC连接数
-- ,'{*_*}' 容量类
 ,sum(M8006C224) QCI1最大用户数
 ,sum(M8006C225) QCI2最大用户数 
,sum(拥塞次数) 拥塞次数
...
,case when round(sum(上行有效RRC连接平均数),2)>round(sum(下行有效RRC连接平均数),2) then round(sum(上行有效RRC连接平均数),2) else round(sum(下行有效RRC连接平均数),2) end 有效RRC连接平均数
,case when sum(上行有效RRC连接最大数)>sum(下行有效RRC连接最大数) then sum(上行有效RRC连接最大数) else sum(下行有效RRC连接最大数) end 有效RRC连接最大数
