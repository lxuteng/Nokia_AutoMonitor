SELECT
substr(sdate,1,10) sdate , city--- ,enb_cell,enb_id,bts_version,cel_name

,round(100*decode(sum(M8020C6-M8020C4),0,0,sum(M8020C3)/sum(M8020C6-M8020C4)),2) 可用率
...
,Round(sum(M8012C19)/(1024*1024*1024),2)   用户面PDCP上行数据量GB
,Round(sum(M8012C20)/(1024*1024*1024),2)   用户面PDCP下行数据量GB
...
,sum(M8020C3) 可用率分子
,sum(M8020C6-M8020C4) 可用率分母
,sum(interference105)  "干扰小区大于-105"
,sum(interference110)  "干扰小区大于-110"
,decode(avg(M8001C217),0,0,round(100*avg(M8011C50avg)/(60*60*1000/5)/avg(M8001C217),2)) 上行PRB利用率  
,round(avg(M8011C37)/10,2) 下行PRB利用率
,sum(M8016C54) esrvcc请求次数16A
--,decode(avg(M8001C217),0,0,round(100*avg(M8011C50avg)/(60*60*1000/5)/avg(M8001C217),2)) 上行PRB平均利用率new  
--,decode(avg(M8001C216),0,0,round(100*avg(M8011C37)/10/avg(M8001C216),U2)) 下行PRB平均利用率new
FROM
(
SELECT M8012.sdate
--   ,M8020.MRBTS_ID
...
,M8051C62,M8051C63,M8001C153,M8001C154,M8026C254,M8026C259,M8006C269,M8006C273, M8020C3 ,M8020C4  ,M8020C6
...
from
NOKLTE_PS_LCELLT_lncel_hour PMRAW,ctp_common_objects lnbts,ctp_common_objects lncel
where
...
WHERE  enb_id between  '0' and '999999'
     /*
(enb_id>='655360' and enb_id<='656383' )
or (enb_id>= '686080' and enb_id<='686591' )
or (enb_id>= '696320' and enb_id<='696831' )
or (enb_id>= '119296' and enb_id<='120831' )
or (enb_id>= '198912' and enb_id<='199423' )
or (enb_id >='809728' and enb_id <='812543')
or (enb_id >='838912' and enb_id <='839423')
or (enb_id >='335872' and enb_id <='336639')
or (enb_id >='557056' and enb_id <='557567')
or (enb_id >= '567552' and enb_id <= '568063')
                */
GROUP BY  substr(sdate,1,10), city---,enb_cell,enb_id,bts_version,cel_name
